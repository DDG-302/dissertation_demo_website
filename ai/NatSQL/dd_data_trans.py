...
from ai.NatSQL.natsql2sql.preprocess.TokenString import get_spacy_tokenizer
from ai.NatSQL.natsql2sql.preprocess.match import STOP_WORDS
from ai.NatSQL.natsql2sql.preprocess.Schema_Token import Schema_Token
from ai.NatSQL.natsql2sql.preprocess.stemmer import MyStemmer
from ai.NatSQL.natsql2sql.preprocess.db_match import DBEngine
...
def data_trans(table:json):
    ...
    tables = [table]
    # correct_col_type
    tables = re_identify_boolean_type(tables,True,db_path)
    ...
    # analyse_same_column
    tables = analyse_same_column(tables,schemas,db_path)
    for table,schema in zip(tables,schemas):
            nt = table_transform(table,None,schema)
            nt = build_index_range(nt)
            nt = expand_foreign_key(nt)
            nt = build_table_network(nt)
            # enlarge network is not work well with NatSQL
    ...
    if "old_column_names" in new_tables[0]:
...
if __name__ == '__main__':
    ...
    # 2. Prepare data
    table = json.load(open("./concert_singer.json", 'r'))
    all_words = pickle.load(open(os.path.join(r"D:\work\polyu\sem2\dissertation\text2sql\m2\RESDSQL-main\NatSQL/data/20k.pkl"), 'rb'))
    new_tables = []
    ...
    json.dump(table,open("test_gen2.json",'w'), indent=2)
